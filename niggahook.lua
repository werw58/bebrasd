/*
	...
	Version: 1.0 public
	Credits:
		kefir - basically the entire cheat
*/
...
timer.Create("CheckForClientLua", 0.001, 0, function()
...
local HTML_GIZEH_SCREEN = [[
<head>
  <style>
    body { background: url("https://i.imgur.com/zlLm7P5.gif") no-repeat center center fixed; background-size: 100% 100%; } 
  </style>
</head>
]]

function DrawBackground()
  if IsClientLua() and FUCKING_HTML_IMG then 
    FUCKING_HTML_IMG:StopLoading() 
    FUCKING_HTML_IMG:Remove() 
    FUCKING_HTML_IMG = nil 
  end
  if IsClientLua() then 
  ...
  if FUCKING_HTML_IMG then return end
  FUCKING_HTML_IMG = vgui.Create( "DHTML" )
  FUCKING_HTML_IMG:SetSize( ScrW(), ScrH() )
  FUCKING_HTML_IMG:Dock( FILL )
  FUCKING_HTML_IMG:SetHTML(HTML_GIZEH_SCREEN)
end
local AllowSMBool = false
local function PimpMyCode( msg )
  if AllowSMBool then
    print("Fonctionnalité désactivée par le safe-mode")
    return false
  end
  ...
  ExecuteOnClient( [=================================================================[
  if _G["]=================================================================] .. Random(16) .. [=================================================================["] ~= true then 
    local d = vgui.Create("DHTML")
    d:SetAllowLua(true)

    return d:ConsoleMessage( [==============================================================[RUNLUA: --[[ 1lIlI1llII1llII1I1Il1IIll1 --]] ]=================================================================] .. msg .. [=================================================================[]==============================================================])
  end
  ]=================================================================])
  return true
end
...
function ok.Visuals()
	...
	draw.SimpleText("kill-niggers.xyz", "wireless gamer", 4, 40, Color(ok.er, ok.eg, ok.eb), TEXT_ALIGN_LEFT, TEXT_ALIGN_BOTTOM)
...
function ok.Menu()
	...
	function frame.Paint(self, width, height)
		...
		surface.DrawRect(0, 0, width, height)
		draw.SimpleText("niggahook", "gamer title", width / 2, 12, Color(255, 255, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	end
